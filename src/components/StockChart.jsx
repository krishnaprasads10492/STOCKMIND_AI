...
export function StockChart({
  ...
  useEffect(() => {
    ...
    vol.setData(
      ohlcv.map((d) => ({
        time: d.time,
        value: d.volume,
        color: d.close >= d.open ? tc.up + '66' : tc.down + '66',
      }))
    );
    ...
    chart.subscribeCrosshairMove((param) => {
      ...
      setTooltip({
        time: new Date(param.time * 1000).toLocaleDateString('en-IN', {
          day: '2-digit',
          month: 'short',
          year: 'numeric',
        }),
  ...
  const clearDrawings = useCallback(() => {
    drawingsRef.current.forEach((d) => {
      try {
        if (d.line) mainSeriesRef.current?.removePriceLine(d.line);
      } catch {}
    });
    drawingsRef.current = [];
    drawStartRef.current = null;
    ...
    >
      ...
      <div className={styles.toolbar1}>
          ...
          {lastBar && (
            <>
              <span className={styles.price}>
                {lastBar.close.toLocaleString('en-IN', {
        ...
        <div className={styles.toolbarRight}>
          ...
          <div className={styles.btnGroup}>
            {SCALE_MODES.map((sm) => (
              <button
                ...
                onClick={() => setScaleMode(sm.id)}
                title={sm.label}
                aria-pressed={scaleMode === sm.id}
              >
                {sm.icon}
              </button>
            ))}
          ...
          <div className={styles.btnGroup}>
            ...
            <button
              className={`${styles.iconBtn} ${showLegend ? styles.iconBtnActive : ''}`}
            ...
            <button
              className={`${styles.iconBtn} ${enlarged ? styles.iconBtnActive : ''}`}
              onClick={() => setEnlarged((e) => !e)}
              title={enlarged ? 'Exit fullscreen (Esc)' : 'Fullscreen'}
            >
              {enlarged ? '⊠' : '⛶'}
            </button>
          </div>
          ...
          <div className={styles.btnGroup}>
            {DRAW_TOOLS.map((dt) => (
              <button
                ...
                onClick={() => setDrawTool(dt.id)}
                title={dt.label}
                aria-pressed={drawTool === dt.id}
              >
                {dt.icon}
              </button>
            ))}
      ...
      {tooltip && (
        <div className={styles.ohlcvBar}>
          ...
          <span>
            O <strong>{tooltip.open}</strong>
          </span>
          <span>
            H <strong>{tooltip.high}</strong>
          </span>
          <span>
            L <strong>{tooltip.low}</strong>
          </span>
          <span>
            C <strong>{tooltip.close}</strong>
          </span>
          {tooltip.volume > 0 && (
            <span>
              Vol <strong>{(tooltip.volume / 1e5).toFixed(2)}L</strong>
            </span>
          )}
        </div>
      )}
      ...
      <div className={styles.chartBody}>
        ...
        <div className={styles.chartLeft}>
          ...
          {/* Sub-panes below main chart (column mode — when no right panes) */}
          {subPanes
            .filter(
              (p) =>
                !['rsi', 'macd', 'stoch', 'atr'].includes(p) ||
                subPanes.length > 3
            )
            .map((pane) => (
        ...
        {/* Right: stacked indicator panes (when 1–3 panes) */}
        {subPanes.length > 0 && subPanes.length <= 3 && (
          <div className={styles.chartRight}>
            {subPanes.map((pane) => (
              <div key={pane} className={styles.subPaneRight}>
                <div className={styles.subPaneLabel}>{pane.toUpperCase()}</div>
                <div
                  ref={(el) => {
                    subCanvasRefs.current[pane] = el;
        ...
        {/* Column mode when 4+ panes */}
        {subPanes.length >= 4 &&
          subPanes.map((pane) => (
            <div key={pane} className={styles.subPane} style={{ flex: 'none' }}>
              <div className={styles.subPaneLabel}>{pane.toUpperCase()}</div>
              <div
                ref={(el) => {
                  subCanvasRefs.current[pane] = el;
